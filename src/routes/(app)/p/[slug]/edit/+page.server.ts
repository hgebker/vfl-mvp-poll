...
export const load: PageServerLoad = ({ params, locals, url }) => {
	...
	if (poll.status !== 'upcoming') throw redirect(303, `/p/${poll.slug}/manage`);
	...
	return {
		...
		rosterPlayerIds: getPollRoster(db, poll.id).map((p) => p.id)
	};
};
	...
	default: async ({ request, params, locals, url }) => {
		...
		try {
			...
			if (err instanceof PollNotEditableError) {
				return fail(400, { error: m.edit_error_not_editable() });
			}
			throw err;
		}
